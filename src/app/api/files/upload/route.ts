...
function resolveUniquePath(dir: string, baseName: string, ext = "") {
  ...
  while (true) {
    const name =
      index === 0 ? `${baseName}${ext}` : `${baseName}_${index}${ext}`;
...
export async function POST(request: NextRequest) {
  ...
  if (!file || !rawClient || !kind) {
    return NextResponse.json(
      { error: "Missing file, client or kind" },
      { status: 400 },
    );
  }
  ...
  const baseName = [
    sanitize(customName || path.basename(file.name, ext)),
    stamp,
    ...collaborators.map(sanitize),
    sanitize(uploader),
    sanitize(comment).replace(/\s+/g, "__"),
  ]
    .filter(Boolean)
    .join("__");
  ...
  if (kind === "programmation" && ext === ".zip") {
    ...
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "panel-programmation-"),
    );
