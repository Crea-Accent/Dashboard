    ...
    try {
      ...
      if (data.tickets && Array.isArray(data.tickets)) {
        ...
        for (const ticket of data.tickets) {
          ...
          fs.writeFileSync(
            path.join(ticketFolder, "ticket.json"),
          ...
          if (Array.isArray(pois)) {
            pois.forEach((poi: any, index: number) => {
              ...
              const moveImage = (imagePathKey: string) => {
                if (
                  poi[imagePathKey] &&
                  poi[imagePathKey].includes("/tickets/")
                ) {
                  ...
                  if (fileName) {
                    ...
                    // update path
                    poi[imagePathKey] = poi[imagePathKey].replace(
                      `/tickets/${fileName}`,
                      `/tickets/${ticket.id}/${fileName}`,
                    );
                  }
                }
              };
              ...
              fs.writeFileSync(
                path.join(ticketFolder, `poi_${importance}_${poi.id}.json`),
        ...
        if (fs.existsSync(ticketFile)) {
          ...
          for (const f of files) {
            if (f.startsWith("poi_") && f.endsWith(".json")) {
              pois.push(
                JSON.parse(fs.readFileSync(path.join(ticketFolder, f), "utf8")),
              );
            }
          }
...
export async function POST(req: NextRequest) {
  ...
  if (!client || !ticket) {
    return NextResponse.json(
      { error: "Missing client or ticket" },
      { status: 400 },
    );
  }
    ...
    if (Array.isArray(pois)) {
      pois.forEach((poi: any, index: number) => {
        ...
        fs.writeFileSync(
          path.join(
            ticketFolder,
            `poi_${poiData.importance}_${poiData.id}.json`,
          ),
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (
      ...
      !fs.existsSync(path.join(ticketFolder, "ticket.json"))
    ) {
      return NextResponse.json({ error: "Ticket not found" }, { status: 404 });
    }

    const metadata = JSON.parse(
      fs.readFileSync(path.join(ticketFolder, "ticket.json"), "utf8"),
    );
