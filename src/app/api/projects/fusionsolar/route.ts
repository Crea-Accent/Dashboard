...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!username || !password || !baseUrl) {
      return NextResponse.json(
        { error: "FusionSolar credentials not configured in environment" },
        { status: 500 },
      );
    }
    ...
    if (action === "list") {
      if (
        listCache.data &&
        Date.now() - listCache.timestamp < LIST_CACHE_EXPIRY
      ) {
        return NextResponse.json({
          success: true,
          data: listCache.data,
    ...
    if (
      stationCode &&
      kpiCache[stationCode] &&
      Date.now() - kpiCache[stationCode].timestamp < KPI_CACHE_EXPIRY
    ) {
      return NextResponse.json({
        success: true,
    ...
    if (!kpiRes.ok) {
      return NextResponse.json(
        { error: "Failed to fetch station KPIs" },
        { status: 502 },
      );
    }
