...
export async function GET(req: NextRequest) {
  ...
  if (!technicianUsername) {
    return NextResponse.json(
      { error: "Missing technician parameter" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    for (const project of projects) {
      const projectFolder = path.join(/*turbopackIgnore: true*/ base, project);
      ...
      if (fs.existsSync(ticketsDir)) {
        try {
              ...
              if (fs.existsSync(ticketFile)) {
                ...
                for (const f of files) {
                  if (f.startsWith("poi_") && f.endsWith(".json")) {
                    pois.push(
                      JSON.parse(
                        fs.readFileSync(path.join(ticketFolder, f), "utf8"),
                      ),
                    );
                  }
                }
          ...
          console.error(
            `Error reading tickets directory for project ${project}:`,
            err,
          );
        }
      }
      ...
      if (fs.existsSync(oldFile)) {
        try {
          ...
          console.error(
            `Error reading tickets.json for project ${project}:`,
            err,
          );
        }
      }
    ...
    allTasks.sort((a, b) => {
      ...
      return (
        new Date(b.ticketCreatedAt).getTime() -
        new Date(a.ticketCreatedAt).getTime()
      );
    });
