  ...
  try {
    ...
    if (!contactId && !testEmail) {
      return NextResponse.json(
        { error: "Contact ID or test email is required" },
        { status: 400 },
      );
    }
    ...
    if (testEmail) {
      ...
      inviteIndex = event.invites.findIndex(
        (inv: any) => inv.contactId === contactId,
      );

      if (inviteIndex === -1) {
        return NextResponse.json(
          { error: "Contact is not in the invite list for this event" },
          { status: 400 },
        );
      }
    ...
    if (!token) {
      return NextResponse.json(
        { error: "Postmark API token is not configured" },
        { status: 500 },
      );
    }
    ...
    const emailHtml = await render(
      EventInviteEmail({ event, contact, baseUrl }),
    );
    ...
    if (!testEmail && inviteIndex !== -1) {
      event.invites[inviteIndex].inviteCount =
        (event.invites[inviteIndex].inviteCount || 0) + 1;
