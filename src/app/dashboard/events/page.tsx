...
export default function Page() {
					...
					{data.map((event) => (
						...
						>
							...
							<div className="text-sm text-zinc-500 space-y-1.5">
								...
								{event.location && (
									<p className="flex items-center gap-2">
										<MapPin size={14} className="text-zinc-400" /> {event.location}
									</p>
								)}
								<p className="flex items-center gap-2">
									<Users size={14} className="text-zinc-400" /> {event.invites?.length || 0} Invited
								</p>
							</div>
						</Link>
					))}
	...
	return (
		<div className="space-y-8">
			...
			<EventModal open={modalOpen} onClose={() => setModalOpen(false)} onSuccess={loadEvents} />
		</div>
	);
}
