...
export default function EventDetail() {
  ...
  const companySuggestions = companiesList
    .filter(
      (c) =>
        company &&
  ...
  async function load() {
    try {
      const [eventRes, contactsRes, companiesRes] = await Promise.all([
        fetch(`/api/events/${id}`),
        fetch("/api/contacts"),
      ...
      const res = await fetch(`/api/events/${id}`, {
        ...
        body: JSON.stringify({
          updates: {
            invites: [
              ...currentInvites,
              { contactId, inviteCount: 0, status: "pending", guests: [] },
            ],
          },
        }),
      });
  ...
  async function handleSendAll() {
    const uninvited = invitedContacts.filter(
      (c: any) =>
        (c.inviteData.inviteCount || 0) === 0 &&
        (!c.inviteData.status || c.inviteData.status === "pending"),
    );
  ...
  function handleGenerateSchedule() {
    ...
    if (names.length < 3) {
      toast(
        "error",
        "You need at least 3 confirmed attendees to generate a schedule.",
      );
      return;
    }
  ...
  function exportStyledExcel(data: any[], people: string[]) {
    ...
    data.forEach((round, roundIndex) => {
      ...
      round.groups.forEach((group: any) => {
        ...
        rowCursor +=
          (hasLeader ? 1 + group.people.length : group.people.length) + 1;
      });
    });
    ...
    const finalPeople = [
      ...leaders,
      ...people.filter((p) => !leaders.includes(p)),
    ];
    ...
    ws["!cols"] = [
      ...
      { wch: 28 },
      { wch: 10 },
      ...Array(data.length * 4).fill({ wch: 18 }),
    ];
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link
            ...
            className="p-2 bg-(--background) border border-(--border)/10 rounded-xl hover:bg-(--foreground) transition"
          >
            <ChevronLeft size={20} />
          </Link>
          <div>
            <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100 flex items-center gap-3">
              {event.name}
              {isPast && (
                <span className="px-2 py-0.5 rounded-full bg-zinc-200 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-400 text-xs font-semibold uppercase tracking-wider">
                  Past
                </span>
              )}
              {!isPast && (
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500">
              {invitedContacts.length} people invited
            </p>
          </div>
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={
                isPast ||
                invitedContacts
                  .filter((c: any) => c.inviteData.status === "confirmed")
                  .reduce(
                    (acc: number, c: any) =>
                      acc + 1 + (c.inviteData.guests?.length || 0),
                    0,
                  ) < 3
              }
            >
        ...
        <div className="space-y-2 mt-4">
          {invitedContacts.map((contact: any) => {
            ...
            const isPending =
              !contact.inviteData.status ||
              contact.inviteData.status === "pending";
            ...
            if (isConfirmed) {
              cardBorder =
                "border-emerald-500 dark:border-emerald-500/50 shadow-sm shadow-emerald-500/10";
            ...
            return (
              <div
                key={contact.id}
                className={`flex flex-col p-4 rounded-xl border transition-all ${cardBorder} ${cardBg}`}
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-zinc-500 flex flex-wrap items-center gap-3 mt-1.5">
                      ...
                      {isConfirmed && (
                        <span className="px-2.5 py-0.5 rounded-full bg-emerald-500 text-white font-semibold">
                          Confirmed
                        </span>
                      )}
                      {isRefused && (
                        <span className="px-2.5 py-0.5 rounded-full bg-red-500 text-white font-semibold">
                          Declined
                        </span>
                      )}
                      {isPending && hasBeenMailed && (
                        <span className="px-2.5 py-0.5 rounded-full bg-amber-400 text-amber-950 font-semibold">
                          Pending (Invite Sent)
                        </span>
                      )}
                      {isPending && !hasBeenMailed && (
                        <span className="px-2.5 py-0.5 rounded-full bg-zinc-200 text-zinc-700 dark:bg-zinc-700 dark:text-zinc-300 font-semibold">
                          Pending (Not Sent)
                        </span>
                      )}

                      <span className="text-xs font-medium text-zinc-400 border-l border-zinc-300 dark:border-zinc-600 pl-3">
                        Sent: {contact.inviteData.inviteCount || 0}
                      </span>
                    </div>
                  </div>
                ...
                {contact.inviteData.guests?.length > 0 && (
                  <div className="mt-3 ml-1 pl-4 border-l-2 border-emerald-500/30">
                    <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-500 mb-1.5 uppercase tracking-wider">
                      Additional Guests
                    </p>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500">
                {confirmedNames.length} total attendees assigned.
              </p>
            </div>
            <Button
              onClick={() => exportStyledExcel(scheduleData, confirmedNames)}
              ...
              >
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {round.groups.map((group: any, i: number) => (
                    <div
                      key={i}
                      className="border border-zinc-200 dark:border-zinc-700/50 rounded-xl p-4 bg-white dark:bg-zinc-900 shadow-sm space-y-2"
                    >
                      ...
                      <div className="text-sm text-zinc-600 dark:text-zinc-300 pt-1 leading-relaxed">
                        {group.people.join(", ")}
                      </div>
                    </div>
                  ))}
        ...
        footer={
          <>
            ...
            <Button
              ...
              disabled={saving || !name || !email || !company}
            >
