...
export default function EventDetail() {
	...
	const companySuggestions = companiesList.filter(c => 
		company && 
	...
	async function handleAddInvite() {
		try {
			...
			const res = await fetch(`/api/events/${id}`, {
				method: 'PATCH',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ updates: { invites: [...currentInvites, { contactId, inviteCount: 0, status: 'pending', guests: [] }] } })
			});
	...
	function exportStyledExcel(data: any[], people: string[]) {
		...
		data.forEach((round, roundIndex) => {
			...
			ws[XLSX.utils.encode_cell({ r: 0, c: startCol })] = { v: `SESSIE ${round.round}`, s: headerStyle };
			...
			round.groups.forEach((group: any) => {
				...
				if (hasLeader) {
					ws[XLSX.utils.encode_cell({ r: baseRow, c: startCol - 1 })] = { v: '10min', s: leaderStyle };
					ws[XLSX.utils.encode_cell({ r: baseRow, c: startCol })] = { v: group.location, s: leaderStyle };
					ws[XLSX.utils.encode_cell({ r: baseRow, c: startCol + 1 })] = { v: group.leader, s: leaderStyle };
				} else {
					ws[XLSX.utils.encode_cell({ r: baseRow, c: startCol })] = { v: group.location, s: leaderStyle };
				}
				...
				group.people.forEach((p: string, i: number) => {
					...
					ws[XLSX.utils.encode_cell({ r: row, c: startCol - 1 })] = { v: hasLeader ? '2min' : '4min', s: centerStyle };
					ws[XLSX.utils.encode_cell({ r: row, c: startCol + 1 })] = { v: p, s: centerStyle };
				});
	...
	const invitedContacts = (event.invites || []).map((inv: any) => {
				...
				<div className="flex items-center gap-4">
					...
					<div>
						<h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100 flex items-center gap-3">
							...
							{!isPast && (
								<button onClick={() => setEditModalOpen(true)} className="p-1.5 text-zinc-400 hover:text-(--accent) hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors">
									<Edit2 size={16} />
								</button>
							)}
						</h1>
						...
						<div className="text-sm text-zinc-500 mt-2 flex flex-wrap items-center gap-x-3 gap-y-1">
							...
							{event.welcomeTime && <span className="flex items-center gap-1.5"><Clock size={14} /> Welcome: {event.welcomeTime}</span>}
							{event.startTime && <span className="flex items-center gap-1.5"><Play size={14} /> Start: {event.startTime}</span>}
							{event.endTime && <span className="flex items-center gap-1.5"><Square size={14} /> End: {event.endTime}</span>}
							{event.time && !event.welcomeTime && <span className="flex items-center gap-1.5"><Clock size={14} /> {event.time}</span>}
			...
			<div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm p-6 space-y-4">
				...
				<div className="space-y-2 mt-4">
					...
					{invitedContacts.length === 0 && (
						<div className="text-sm text-zinc-500 py-4">No one has been invited yet.</div>
					)}
				</div>
			</div>
