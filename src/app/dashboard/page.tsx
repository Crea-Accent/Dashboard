...
export default function Home() {
  ...
  const [selectedProject, setSelectedProject] =
    useState<ProjectMapEntry | null>(null);
  ...
  const filteredProjects = useMemo(() => {
    ...
    return searched.map((p) => ({
      ...projects.find((pr) => pr.name == p.name),
      ...searched,
    }));
  }, [mapProjects, query]);
  ...
  const updatedLast7Days = useMemo(() => {
    ...
    return projects.filter(
      (p) => p.modified && new Date(p.modified).getTime() > weekAgo,
    ).length;
  }, [projects]);
    ...
    (async () => {
      try {
        ...
        const res = await fetch(
          `/api/files?view=${encodeURIComponent(s.path)}`,
        );
    ...
    <div className="w-full space-y-10">
		...
		`,
          ...
          <div className="relative z-10 grid md:grid-cols-[1fr_320px] gap-8 items-start min-h-40">
            <div className="space-y-4">
              ...
              >
                Here's what's happening across your projects, installations and
                systems.
              </p>
              ...
              {query &&
                filteredProjects.slice(0, 3).map((p) => {
                  ...
                  return (
                    <Link
                      key={p.path}
                      href={`/dashboard/projects/${encodeURIComponent(p.name as string)}`}
                      onClick={() => pushRecent(p.name as string)}
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="text-(--text-muted)">
                          {p.modified && (
                            <p>{new Date(p.modified).toLocaleDateString()}</p>
                          )}
                        </div>
                      </Button>
                    </Link>
        ...
        <div style={{ height: 500 }}>
          <APIProvider apiKey={process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY!}>
            <Map
              mapId={"map"}
              colorScheme={
                (resolvedTheme.toUpperCase() as ColorScheme) ?? "DARK"
              }
              defaultCenter={{
                lat: 50.85,
                lng: 4.35,
              ...
              <MarkerCluster>
                {(setMarkerRef) =>
                  mapProjects.map((project) => {
                    const isHighlighted =
                      query.trim().length > 0 &&
                      filteredProjects.some((p) => p.name === project.name);
                          ...
                          <div
                            ...
                            style={{
                              ...
                              boxShadow: isHighlighted
                                ? `0 0 20px ${project.color}, 0 4px 12px rgba(0,0,0,0.35)`
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard
          ...
          value={loading ? "—" : updatedLast7Days}
        />
        <EnergyCard />
      </section>
      ...
      <Modal
        ...
        footer={
          selectedProject && (
            <Link
              href={`/dashboard/projects/${encodeURIComponent(selectedProject.name)}`}
            >
          ...
          <div className="space-y-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="font-semibold truncate">
                    {selectedProject.label ?? "No Label"}
                  </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div className="font-medium">
                  {selectedProject.address?.postalCode ?? "-"}
                </div>
              </div>
            </div>
              ...
              >
                ...
                <div>
                  {[
                    ...
                    selectedProject.address.number,
                  ]
                    .filter(Boolean)
                    .join(" ")}
                </div>
