...
export default function ProjectPage({
  ...
  const [metadataActions, setMetadataActions] =
    useState<MetadataActions | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      const [s, m, p] = await Promise.all([
        ...
        fetch(
          `/api/projects/metadata?client=${encodeURIComponent(id)}&reveal=true`,
        )
          .then((r) => r.json())
          .catch(() => null),
      ...
      const initialView = new URL(window.location.href).searchParams.get(
        "view",
      ) as Tab;
  ...
  if (loading)
    return (
      <Loading
        title={`Loading ${client || "project"}`}
        description="Reading project metadata"
      />
    );
  ...
  if (!settings?.path)
    return (
      <EmptyState
        title="Projects path not configured"
        description="Configure a base projects path in settings before opening project data."
      />
    );
      ...
      <div className="space-y-6">
        ...
        <div>
          <Selector
            ...
            onChange={(val) =>
              router.push(`/dashboard/projects/${encodeURIComponent(val)}`)
            }
          />
        ...
        >
          <div
            className={`flex items-center gap-2 ${selectorOpen ? "hidden sm:flex" : ""}`}
          >
            {tab === "info" && (
              <Button
                icon={
                  metadataActions?.saved ? (
                    <Check size={16} />
                  ) : (
                    <Save size={16} />
                  )
                ...
                disabled={
                  !metadataActions?.hasChanges ||
                  metadataActions?.saving ||
                  !isAllowed
                }
            ...
            <Button
              icon={<MapPin size={16} />}
              onClick={openMaps}
              disabled={!metadata?.address?.city}
            >
        ...
        >
          {tab === "info" && (
            <Metadata client={client} onActionsChange={setMetadataActions} />
          )}
          {tab === "solar" && <Solar client={client} />}
          {tab === "schemas" && (
            <Schemas basePath={settings.path} client={client} />
          )}
          {tab === "documents" && (
            <Documents basePath={settings.path} client={client} />
          )}
          {tab === "programmation" && (
            <Programmation basePath={settings.path} client={client} />
          )}
          {tab === "canbus" && (
            <Canbus basePath={settings.path} client={client} />
          )}
          {tab === "pictures" && (
            <Pictures basePath={settings.path} client={client} />
          )}
