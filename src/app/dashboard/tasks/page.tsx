...
import { CheckCircle2, Circle, Clock, ClipboardList, FolderOpen, ImageIcon, Search } from 'lucide-react';
...
export default function TasksPage() {
	...
	const load = async () => {
		...
		try {
			setLoading(true);
			const res = await fetch(`/api/tasks?technician=${encodeURIComponent(username)}`);
			if (res.ok) {
	...
	const markPOIDone = async (task: Task, finishedImagePath?: string, completedBy?: string, proofDescription?: string) => {
		// Optimistic update
		const updatedTasks = tasks.map((t) => (t.id === task.id ? { ...t, state: 'finished' as const, finishedImagePath, completedBy, proofDescription } : t));
			...
			if (res.ok) {
				...
				if (ticket) {
					const updatedPOIs = ticket.pois.map((poi: any) =>
						poi.id === task.id ? { ...poi, state: 'finished' as const, finishedImagePath, completedBy, proofDescription } : poi
					);
	...
	if (!has('tasks.read') && !has('tasks.write')) return <EmptyState title='Access Denied' description='You do not have permission to view tasks.' />;
			...
			<div className='flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3'>
				...
				<div className='flex flex-col sm:flex-row gap-3 w-full sm:w-auto'>
					<MultiSelector
						...
						options={[
							{ label: 'Uncompleted', value: 'unfinished', color: 'var(--accent)' },
							{ label: 'Completed', value: 'finished', color: 'var(--green-500, #22c55e)' },
						]}
					/>
							...
							<Card className='flex flex-col h-full hover:border-(--accent) transition-colors overflow-hidden min-w-0'>
								<div className='p-4 flex-1 space-y-4 min-w-0'>
									<div className='flex items-start justify-between gap-2 min-w-0'>
										...
										<span
											className='px-2 py-0.5 rounded-full text-xs font-medium capitalize shrink-0 bg-zinc-500/10 text-zinc-500 font-mono'>
											#{task.importance || '-'}
										</span>
									</div>

									<div className={`text-sm break-words whitespace-normal space-y-1`}>
										<p className={`${task.state === 'finished' ? 'line-through text-(--text-muted)' : ''}`}>
											{task.description}
										</p>
									...
									{task.imagePath && (
										<div className='relative w-full overflow-hidden min-w-0'>
											<File 
												file={{ name: task.imagePath.split(/[/\\]/).pop() || 'POI.jpg', path: task.imagePath, type: 'file' }} 
												image 
											/>
										</div>
									)}
									...
									{task.finishedImagePath && (
										<div className='relative w-full overflow-hidden min-w-0'>
											<File 
												file={{ name: task.finishedImagePath.split(/[/\\]/).pop() || 'Proof.jpg', path: task.finishedImagePath, type: 'file' }} 
												image 
											/>
									...
									<div className='text-xs text-(--text-muted) flex items-center gap-2 min-w-0 flex-wrap'>
										...
										<span className='truncate min-w-0 flex-1'>Opened {new Date(task.ticketCreatedAt).toLocaleDateString()} by {task.ticketOpenedBy}</span>
									</div>
								</div>

								<div className='p-3 border-t border-(--border)/10 bg-(--background) flex items-center justify-between min-w-0'>
