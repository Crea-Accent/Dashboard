...
import { CheckCircle2, Circle, Clock, ClipboardList, FolderOpen, ImageIcon, Search, Edit3 } from 'lucide-react';
							...
							<Card className="flex flex-col h-full hover:border-(--accent) transition-colors overflow-hidden min-w-0">
								...
								<div className="p-3 border-t border-(--border)/10 bg-(--background) flex items-center justify-between min-w-0 gap-2">
									<div className="flex items-center gap-2">
										<div className="text-xs font-medium text-(--text-muted)">{task.state === 'finished' ? 'Completed' : 'Pending'}</div>
										{has('tasks.write') && (
											<button onClick={() => setTaskToEdit(task)} className="text-(--text-muted) hover:text-(--accent) transition-colors p-1" title="Edit Task">
												<Edit3 size={14} />
											</button>
										)}
									</div>
