...
function buildGroups(
  ...
  seenLocations: Map<string, Set<string>>,
): string[][] {
  let best: string[][] = [];
    ...
    for (const person of people) {
      ...
      for (const loc of requiredLocations) {
        ...
        for (const round of schedule) {
          ...
          const currentGroup = round.groups.find((g) =>
            g.people.includes(person),
          );
          ...
          for (let i = 0; i < targetGroup.people.length; i++) {
            ...
            if (candidateMissing <= personMissing) {
              ...
              currentGroup.people[currentGroup.people.indexOf(person)] =
                candidate;
