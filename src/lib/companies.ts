			...
			if (Array.isArray(parsed)) {
				for (const company of parsed) {
					if (company && company.id) {
						await fs.writeFile(path.join(companiesDir, `${company.id}.json`), JSON.stringify(company, null, 2), 'utf8');
					}
				}
			}
...
export async function getCompany(id: string): Promise<Company | null> {
	await ensureMigrated();
	try {
		const content = await fs.readFile(path.join(companiesDir, `${id}.json`), 'utf8');
