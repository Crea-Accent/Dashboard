    ...
    try {
      ...
      const poll = async () => {
        try {
          ...
          const importLifetime =
            (await readRegister(client, 37119, 2, 1)) / 100;

          const exportLifetime =
            (await readRegister(client, 37121, 2, 1)) / 100;
          ...
          const batteryDischarge =
            (await readRegister(client, 37068, 2, 1)) / 100;
          ...
          global.latestModbusData = {
            ...
            state: {
              inverter: inverterStatus
                ? (inverterStates[inverterStatus] ?? "Standby")
                : "Offline",
