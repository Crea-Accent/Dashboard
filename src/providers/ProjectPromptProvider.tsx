...
const ProjectPromptContext = createContext<ProjectPromptContextType | null>(
  null,
);
...
export function ProjectPromptProvider({
  children,
}: {
  ...
  useEffect(() => {
    ...
    const alreadyViewing = nearbyProjects.some((project) =>
      pathname.startsWith(
        `/dashboard/projects/${encodeURIComponent(project.name)}`,
      ),
    );
  ...
  return (
    <ProjectPromptContext.Provider value={{}}>
      ...
      {session &&
        session?.user.preferences?.projectPrompts &&
        has("projects.read") && (
          <Modal
            open={open}
            title={
              nearbyProjects.length === 1
                ? "Nearby Project Found"
          ...
          >
            <div className="space-y-4">
              <div className="text-sm text-(--text-muted)">
                {nearbyProjects.length === 1
                  ? "You appear to be at a project location."
                  : "You appear to be near multiple project locations."}
              </div>
              ...
              <div className="flex justify-between items-center pt-2">
                <span className="text-xs text-(--text-muted)">
                  {nearbyProjects.length} nearby project
                  {nearbyProjects.length === 1 ? "" : "s"}
                </span>
