...
export default function Metadata({ client, onActionsChange }: Props) {
  ...
  const [initialMetadata, setInitialMetadata] = useState<MetadataType | null>(
    null,
  );
  ...
  const save = async () => {
    ...
    const refreshed = await fetch(
      `/api/projects/metadata?client=${client}`,
    ).then((r) => r.json());
  ...
  const hasChanges =
    JSON.stringify(normalize(metadata)) !==
    JSON.stringify(normalize(initialMetadata));
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch(
        `/api/projects/metadata?client=${encodeURIComponent(client)}&reveal=true`,
      );
  ...
  if (!metadata)
    return (
      <EmptyState
        title="No metadata"
        description="No metadata could be loaded for this project."
      />
    );
                  ...
                  {showSuggestions &&
                    projectNames.filter(
                      (n) =>
                        n
                          .toLowerCase()
                          .includes((metadata.project ?? "").toLowerCase()) &&
                        n !== metadata.project,
                    ).length > 0 && (
                      <motion.div
