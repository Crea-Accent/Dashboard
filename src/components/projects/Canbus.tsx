...
export default function Canbus({ basePath, client }: Props) {
  ...
  const matches = useMemo(() => {
    ...
    function traverse(tree: TopologyModule[]) {
      for (const entry of tree) {
        ...
        const node = foundModules.find(
          (n) => n.physicalAddress === entry.physicalAddress,
        );
        ...
        if (node) {
          ...
          if (node.nodeAddress?.toString().toLowerCase().includes(term))
            isMatch = true;
          if (
            node.units?.some(
              (u: any) =>
                u.name?.toLowerCase().includes(term) ||
                u.unitTypeName?.toLowerCase().includes(term),
            )
          ) {
            isMatch = true;
          }
  ...
  const [moduleSelection, setModuleSelection] =
    useState<ModuleSelection | null>(null);
  ...
  const unplacedModules = foundModules.filter(
    (node) => !containsPhysicalAddress(topology, node.physicalAddress),
  );
  ...
  function containsPhysicalAddress(
    tree: TopologyModule[],
    address: string,
  ): boolean {
  ...
  async function executeInsertion(module: TopologyModule) {
    if (!insertTarget || insertTarget.type === "end") {
      ...
      insertTarget.type === "branch_start" &&
  ...
  function removeRecursive(
    ...
    return tree
      ...
      .map((module) => ({
        ...
        nodes: module.nodes
          ? Object.fromEntries(
              Object.entries(module.nodes).map(([branch, children]) => [
                branch,
                removeRecursive(children, id),
              ]),
            )
          : undefined,
      }));
    ...
    return tree.map((module) => ({
      ...
      nodes: module.nodes
        ? Object.fromEntries(
            Object.entries(module.nodes).map(([branch, children]) => [
              branch,
              updateRecursive(children, id, moduleId),
            ]),
          )
        : undefined,
    }));
  ...
  async function changeModuleType(module: ModuleDefinition) {
    ...
    const next = updateRecursive(
      topology,
      moduleSelection.topology.instanceId,
      module.id,
    );
  ...
  async function load() {
    try {
      ...
      const files = await fetch(
        `/api/files?view=${encodeURIComponent(programmationPath)}&recursive=1`,
      ).then((r) => r.json());
      ...
      const projectFolders: Array<string> = Array.from(
        new Set(
          files
            .filter(
              (f: any) =>
                f.type === "file" && f.name.toLowerCase().endsWith(".duo"),
            )
            .map((f: any) => f.path.split(/[\\/]/).slice(0, -1).join("/")),
        ),
      );
      ...
      const nodeDatabase = await fetch(
        `/api/files/download?path=${encodeURIComponent(`${latestFolder}/Config/nodedatabase.cache.json`)}`,
      ).then((r) => r.json());

      const moduleDefinitions = await fetch("/api/projects/modules").then((r) =>
        r.json(),
      );
  ...
  function isBeforeSwitch(tree: TopologyModule[], id: string): boolean {
    for (let i = 0; i < tree.length - 1; i++) {
      if (
        tree[i].instanceId === id &&
        (tree[i + 1].moduleId === "DT00-24SW" ||
          tree[i + 1].moduleId === "DT13-SW")
      ) {
        return true;
      }
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="rounded-lg p-1 flex items-center justify-center h-[160px] shrink-0">
            <ReactSVG
              src={`/modules/${module.id}/drawing.svg`}
          ...
          <div className="px-3 pb-3 space-y-3">
            ...
            <div className="flex flex-wrap justify-end gap-2">
              {!!node?.units?.length && module.detectable && (
                <Button
                  variant="ghost"
                  icon={<ListTree size={14} />}
                  onClick={() => setUnitModal(node)}
                />
              )}
              ...
              <Button
                ...
                onClick={() =>
                  window.open(`/modules/${module.id}/datasheet.pdf`, "_blank")
                }
              />
                  ...
                  <div className="flex justify-center">
                    {canMoveUp && (
                      <Button
                        ...
                        onClick={() => moveModuleToBranch(entry.instanceId, -1)}
                      />
                    )}
                  </div>
                  <div />
                  <div className="flex justify-center">
                    {canMoveLeft && (
                      <Button
                        ...
                        onClick={() => moveModule(entry.instanceId, "up")}
                      />
                    )}
                  </div>
                  <div />
                  <div className="flex justify-center">
                    {canMoveRight && (
                      <Button
                        ...
                        onClick={() => moveModule(entry.instanceId, "down")}
                      />
                    )}
                  </div>
                  <div />
  ...
  function renderSerialLine(
    line: TopologyModule[] | undefined,
  ): React.ReactNode {
  ...
  function renderTopologyNode(
    entry: TopologyModule,
    serialRest: TopologyModule[],
  ): React.ReactNode {
    const module = availableModules.find((m) => m.id === entry.moduleId);
    const branchCount = module?.channels ?? 0;
    ...
    return (
      <div key={entry.instanceId} className="flex flex-row items-start">
          ...
          {branches.length > 0 && (
            <div className="flex flex-col relative w-0 items-start mt-4 overflow-visible">
              {branches.map((branch, i) => (
                <div
                  key={i}
                  className="flex flex-row items-start relative pt-8 pb-4 w-max"
                >
                  ...
                  <div
                    ...
                    style={{
                      ...
                      top: i === 0 ? "-1rem" : "0",
                      bottom:
                        i === branches.length - 1 ? "calc(100% - 10rem)" : "0",
                    }}
                  >
                  ...
                  <div
                    className="w-12 absolute flex flex-col justify-between h-2.5 group"
                    style={{ left: "140px", top: "10rem" }}
                  >
        ...
        {(serialRest.length > 0 || editing) && (
          <>
            <div
              className={`w-12 relative flex flex-col justify-between h-2.5 flex-shrink-0 group ${serialRest.length === 0 ? "opacity-50" : ""}`}
  ...
  return (
    <div className="flex flex-col flex-1 w-full">
      <div
        id="canbus-topology-container"
        className="mt-8 w-full overflow-x-auto pb-64 pl-8 flex-1"
      >
        <div
          className="flex flex-row items-start min-w-max relative"
          style={{ zoom }}
        >
        ...
        createPortal(
          <>
            <div className="relative flex items-center w-full sm:w-auto min-w-[200px] sm:min-w-[300px]">
              ...
              {search && matches.length === 0 && (
                <div className="absolute right-4 text-sm text-[var(--text-muted)] z-10 font-medium">
                  0/0
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3 justify-center w-full sm:w-auto">
              {has("projects.write") && (
                <Button
                  ...
                  icon={editing ? <Save size={16} /> : <Pencil size={16} />}
                />
              )}
              ...
              <div className="flex items-center gap-1">
                ...
                />
                <span className="text-xs sm:text-sm font-semibold w-10 sm:w-12 text-center text-[var(--text)]">
                  {Math.round(zoom * 100)}%
                </span>
                <Button
                  variant="ghost"
                ...
                <Button
                  ...
                  onClick={async () => {
                    const container = document.getElementById(
                      "canbus-topology-container",
                    );
                    if (!container) return;

                    const innerContent = container.querySelector(
                      ".flex-row.items-start.min-w-max",
                    );
                    ...
                    try {
                      ...
                      tiler.style.cssText =
                        "position: absolute; top: 0; left: 0; z-index: -100; opacity: 0; pointer-events: none;";
                      ...
                      for (let i = 0; i < numPages; i++) {
                        ...
                        const clone = innerContent.cloneNode(
                          true,
                        ) as HTMLElement;
                        clone.style.cssText = `
        ...
        <div className="space-y-8 max-h-[70vh] overflow-y-auto pr-2">
          <div>
            <h3 className="mb-4 text-lg font-semibold">
              Detected Modules ({unplacedModules.length})
            </h3>

            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              {unplacedModules.map((node) => (
                <Card
                  key={node.physicalAddress}
                  onClick={() => selectDetectedModule(node)}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              {manualModules.map((module, i) => (
                <Card
                  key={i}
                  onClick={() => addManualModule(module)}
                  className="cursor-pointer transition hover:scale-[1.02]"
                >
                  <div className="flex flex-col gap-4 p-2">
      ...
      <Modal
        ...
        size={"xxl"}
        onClose={() => setModuleSelection(null)}
        title="Select Module Type"
      >
        <div className="space-y-6">
          <div>
            <h3 className="font-semibold">
              {moduleSelection?.mode === "add"
                ? moduleSelection.node.name
                : availableModules.find(
                    (x) => x.id === moduleSelection?.topology.moduleId,
                  )?.name}
            </h3>

            <p className="opacity-70">
              {moduleSelection?.mode === "add"
                ? moduleSelection.node.physicalAddress
                : moduleSelection?.topology.physicalAddress}
              {" - "}
              {moduleSelection?.mode === "add" &&
                moduleSelection?.node?.nodeAddress}
            </p>
          </div>
                ...
                <Card key={index}>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm opacity-70">
                        Channel {unit.unitAddress}
                      </p>
                    </div>

                    <div className="text-right text-sm opacity-70">
                      <div>
                        {units[unit.unitTypeName as keyof typeof units] ||
                          unit.unitTypeName}
                      </div>
                    </div>
                  </div>
