...
export default function Pictures({
  ...
  const load = async () => {
    try {
      ...
      const [filesRes, usersRes] = await Promise.all([
        fetch(
          `/api/files?view=${encodeURIComponent(picturesPath)}&recursive=1`,
        ),
        fetch("/api/users"),
      ]);
      ...
      const hasUngrouped = fileData.some(
        (file) =>
          file.type === "file" &&
          MEDIA_EXTENSIONS.some((ext) =>
            file.name.toLowerCase().endsWith(ext),
          ) &&
          getFolderName(file) === "Ungrouped",
      );
  ...
  const uploadWithMetadata = async (
    ...
    const targetDir =
      group && group !== "Ungrouped" ? `picture/${group}` : "picture";
  ...
  const moveFileToGroup = async (file: FileEntry | null, group: string) => {
    ...
    const targetDir =
      group === "Ungrouped"
        ? `${basePath}/${client}/picture`
        : `${basePath}/${client}/picture/${group}`;
      ...
      <div className="rounded-3xl bg-(--foreground) p-6 space-y-6">
        ...
        {groups.length === 0 && files.length === 0 && (
          <EmptyState
            ...
            description="Upload media or create a media group to get started."
          />
        )}
        ...
        {groups.map((group) => {
          ...
          const folderFiles = files.filter(
            (file) => getFolderName(file) === folder,
          );
          ...
          return (
            <div
              ...
              onDrop={async (e) => {
                ...
                const droppedOSFiles = Array.from(
                ...
                <div className="rounded-3xl p-6 min-h-20 flex items-center justify-center border-2 border-dashed border-(--accent)/30 bg-(--background)">
                  <div className="text-center">
                    ...
                    <div className="text-xs text-(--text-muted) mt-1 opacity-70">
                      Drag images here or upload new ones
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="p-12 text-center bg-(--foreground) rounded-2xl shadow-xl border border-(--border)/10">
            <div className="space-y-3">
              ...
              <p className="text-sm text-(--text-muted)">
                Release your media anywhere to upload them, or drop on a group
                to upload directly.
              </p>
            </div>
          </div>
