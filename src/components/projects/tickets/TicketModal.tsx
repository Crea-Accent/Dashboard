...
export default function TicketModal({
  ...
  const handlePOIChange = (id: string, field: keyof POIState, value: any) => {
    setPOIs((curr) =>
      curr.map((p) => (p.id === id ? { ...p, [field]: value } : p)),
    );
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPOIs((curr) =>
      curr.map((p) =>
        p.id === poiId ? { ...p, file, filePreview: preview } : p,
      ),
    );
  ...
  const handleSave = async () => {
    try {
      ...
      if (existingTicket) {
        const updatedPOIs = [...existingTicket.pois, ...finalPOIs].map(
        ...
        <>
          ...
          <Button
            ...
            disabled={
              saving ||
              uploading ||
              (!existingTicket && !name.trim()) ||
              pois.some((p) => !p.description.trim())
            }
          >
            {saving || uploading
              ? "Saving..."
      ...
      <div className="space-y-6 max-h-[70vh] overflow-y-auto px-1">
        ...
        <div className="space-y-4">
          ...
          {pois.map((poi, index) => (
            <div
              key={poi.id}
              className="bg-(--foreground) p-4 rounded-2xl border border-(--border)/10 space-y-4 relative"
            >
              {pois.length > 1 && (
                <button
              ...
              <div>
                <Input
                  ...
                  onChange={(e) =>
                    handlePOIChange(poi.id, "description", e.target.value)
                  }
              ...
              <div className="py-2 border-t border-(--border)/10">
                <Toggle
                  ...
                  onChange={(checked) =>
                    handlePOIChange(poi.id, "requiresPicture", checked)
                  }
                ...
                {poi.filePreview && (
                  <div className="relative">
                    <img
                      src={poi.filePreview}
                      alt="Preview"
                      className="h-12 w-12 object-cover rounded-lg border border-(--border)/10"
                    />
                    <button
                      onClick={() => {
