...
export default function CompleteTaskModal({ open, client, poiId, requiresPicture, users = [], ticketId, onClose, onComplete }: CompleteTaskModalProps) {
	...
	return (
		<Modal
			...
			footer={
				<>
					<Button variant="secondary" onClick={handleClose} disabled={saving || uploading}>
						Cancel
					</Button>
					<Button onClick={handleSave} disabled={saving || uploading || (requiresPicture && !file)}>
		...
		>
			<div className="space-y-4">
				<p className="text-sm text-(--text-muted) mb-4">
					You are about to mark this task as completed. {requiresPicture ? 'Proof media is required for this task.' : 'You can optionally attach media as proof of work.'}
				</p>

				<input type="file" accept="image/*" capture="environment" className="hidden" ref={fileInputRef} onChange={handleFileChange} />

				<div className="flex items-center gap-4 pt-2">
					<Button variant="secondary" size="sm" icon={<ImageIcon size={16} />} onClick={() => fileInputRef.current?.click()}>
						{file ? 'Change Photo' : 'Take / Upload Proof Photo'}
					</Button>

					{filePreview && (
						<div className="relative">
							<img src={filePreview} alt="Preview" className="h-16 w-16 object-cover rounded-lg border border-(--border)/10" />
							<button
								onClick={() => {
									setFile(null);
									setFilePreview(null);
