...
function getOffsetsInMeters(
  ...
  const x =
    lngDiff * (Math.PI / 180) * EARTH_RADIUS * Math.cos((lat1 * Math.PI) / 180);
  return { x, y };
}

function applyOffsetsInMeters(
  lat1: number,
  lng1: number,
  x: number,
  y: number,
  ...
  const lng2 =
    lng1 +
    (x / (EARTH_RADIUS * Math.cos((lat1 * Math.PI) / 180))) * (180 / Math.PI);
...
function snapToGrid(
  ...
  const snappedX =
    snappedGridX * Math.cos(rotateAngle) - snappedGridY * Math.sin(rotateAngle);
  const snappedY =
    snappedGridX * Math.sin(rotateAngle) + snappedGridY * Math.cos(rotateAngle);
...
export default function Solar({ client }: Props) {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "c") {
        if (selectedPanels.length > 0) {
          const copied = selectedPanels
            .map((idx) => customPanels[idx])
            .filter(Boolean);
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        if (selectedPanels.length > 0) {
          setCustomPanels((prev) =>
            prev.filter((_, i) => !selectedPanels.includes(i)),
          );
          setSelectedPanels([]);
          setContextMenu(null);
        }
  ...
  async function analyzeRoof() {
    try {
      ...
      if (data.error) {
        ...
        setError(
          "Google Solar API could not analyze this roof. Defaulting to custom layout mode.",
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    const index = configs.findIndex(
      (x: any) => x?.panelsCount === solar.recommended?.panelsCount,
    );
  ...
  const recommendedPanels =
    solar?.solarPanels?.filter((panel: any) => {
      const summary = selectedConfig?.roofSegmentSummaries?.find(
        (x: any) => x.segmentIndex === panel.segmentIndex,
      );
      if (!summary) return false;
      const panelsInSegment = solar.solarPanels.filter(
        (p: any) => p.segmentIndex === panel.segmentIndex,
      );
    ...
    if (customPanels.length === 0 && recommendedPanels.length > 0) {
      setCustomPanels(
        recommendedPanels.map((p: any) => ({
          ...
          azimuth:
            selectedConfig?.roofSegmentSummaries?.find(
              (x: any) => x.segmentIndex === p.segmentIndex,
            )?.azimuthDegrees ?? 0,
        })),
      );
    }
    ...
    if (debugGrid && customPanels.length > 0) {
      ...
      for (let r = -10; r <= 10; r++) {
        for (let c = -10; c <= 10; c++) {
          ...
          const snappedX =
            gridX * Math.cos(rotateAngle) - gridY * Math.sin(rotateAngle);
          const snappedY =
            gridX * Math.sin(rotateAngle) + gridY * Math.cos(rotateAngle);

          const pos = applyOffsetsInMeters(
            anchor.lat,
            anchor.lng,
            snappedX,
            snappedY,
  ...
  if (loading)
    return (
      <Loading
        title="Loading Solar Analyzer"
        description="Checking for existing solar configuration."
      />
    );
  ...
  if (customMode && solar?.recommended) {
    const yieldPerPanel =
      solar.recommended.yearlyEnergyDcKwh / solar.recommended.panelsCount;
    ...
    >
        ...
        <div>
          ...
          <div className="text-sm text-(--text-muted)">
            ...
            {solar?.maxSunshineHoursPerYear && (
              <>
                {" • "}
                {Math.round(solar.maxSunshineHoursPerYear).toLocaleString()} sun
                hours/year
              </>
            )}
          </div>
        ...
        createPortal(
          <>
            ...
            {has("projects.write") && (
              <Button
                loading={calculating}
                disabled={hasAnalysis && isCustomActive}
                onClick={analyzeRoof}
      ...
      {!hasAnalysis && (
        <EmptyState
          ...
          description={
            error ||
            "Run a roof analysis to calculate possible panel layouts, yearly production estimates and optimal panel placement."
          }
        />
      )}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 space-y-6">
            <APIProvider apiKey={process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY!}>
              <MapController
                onMapLoaded={(m) => (mapRef.current = m)}
              ...
              <motion.div
                ...
                onMouseDownCapture={(e) => {
                  ...
                  if ((e.target as HTMLElement).closest(".solar-panel-marker"))
                    return;
                  ...
                  if (isMarqueeSelecting) {
                    ...
                    if (e.shiftKey || e.ctrlKey || e.metaKey) {
                      setSelectedPanels((prev) =>
                        Array.from(new Set([...prev, ...newSelected])),
                      );
                    } else {
              ...
              >
                ...
                >
                  ...
                  {visiblePanels.map((panel: any, index: number) => {
                    ...
                    const lat = isCustomActive
                      ? panel.lat
                      : panel.center.latitude;
                    const lng = isCustomActive
                      ? panel.lng
                      ...
                      <AdvancedMarker
                        ...
                        onDragEnd={(e) => {
                          ...
                          customPanels.forEach((p, i) => {
                            if (i === index) return;
                            const offsets = getOffsetsInMeters(
                              p.lat,
                              p.lng,
                              lat,
                              lng,
                            ...
                            const dist = Math.sqrt(
                              offsets.x * offsets.x + offsets.y * offsets.y,
                            );
                            if (dist < minDistance) {
                              minDistance = dist;
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div>
                      <div className="text-3xl font-bold text-green-500">
                        {fusionData.dataItemMap?.active_power || 0}
                      </div>
                    ...
                    <div>
                      <div className="text-3xl font-bold">
                        {fusionData.dataItemMap?.day_power || 0}
                      </div>
                      <div className="text-sm text-(--text-muted) mt-1">
                        Today (kWh)
                      </div>
                    ...
                    <div>
                      <div className="text-3xl font-bold">
                        {fusionData.dataItemMap?.total_power || 0}
                      </div>
                      <div className="text-sm text-(--text-muted) mt-1">
                        Total (kWh)
                      </div>
          ...
          <div className="space-y-6">
            <div className="rounded-3xl bg-(--foreground) overflow-hidden">
              ...
              >
                ...
                <ChevronDown
                  size={18}
                  className={`transition-transform duration-200 ${configExpanded ? "rotate-180" : ""}`}
                />
              </div>
                    ...
                    <div className="p-6 pt-0">
                      <div className="mb-6 pb-6 border-b border-(--border)/20">
                        <Toggle
                          ...
                          description={
                            isForcedCustom
                              ? "Google API missing roof data. Forced custom mode."
                              : "Manually place and configure panels"
                          }
                        />
                      </div>
                      ...
                      {!isCustomActive && solar?.configs?.length > 0 && (
                        <div className="space-y-4">
                          <p className="text-sm text-(--text-muted) mb-2">
                            Google Solar API Recommended Configuration:
                          </p>
                          <input
                            type="range"
                            min={0}
                          ...
                          <div className="flex justify-between text-xs text-(--text-muted)">
                            ...
                            <span>
                              {configs.at(-1)?.panelsCount ?? 0} panels
                            </span>
                          </div>
                        </div>
                      )}
                      ...
                      {isCustomActive && (
                        <div className="space-y-4">
                          <p className="text-sm text-(--text-muted) mb-4">
                            Right-click on the map to add panels. Right-click on
                            a panel to delete or rotate.
                          </p>
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              ...
                              <input
                                ...
                                value={panelWattage}
                                onChange={(e) =>
                                  setPanelWattage(Number(e.target.value))
                                }
                            ...
                            <div>
                              ...
                              <input
                                ...
                                value={panelLength}
                                onChange={(e) =>
                                  setPanelLength(Number(e.target.value))
                                }
                                className="w-full bg-(--background) rounded-lg px-3 py-2 text-sm border border-(--border)/20 focus:border-(--accent) outline-none"
                              />
                            </div>
                          ...
                          <div className="pt-4 mt-2 border-t border-(--border)/20">
                            ...
                            <div className="flex gap-2">
                              <input
                                ...
                                value={panelsToAdd}
                                onChange={(e) =>
                                  setPanelsToAdd(Number(e.target.value))
                                }
                                ...
                                onClick={() => {
                                  ...
                                  const centerLat =
                                    customPanels.length > 0
                                      ? customPanels[customPanels.length - 1]
                                          .lat
                                      : address.lat;
                                  const centerLng =
                                    customPanels.length > 0
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-(--text-muted)">
                              ≈{" "}
                              {customMode
                                ? (totalCustomWattage / 1000).toFixed(2)
                                : (
                                    (selectedConfig?.panelsCount * 440) /
                                    1000
                                  ).toFixed(1)}{" "}
                              kWp
                            </div>
            ...
            <div className="rounded-3xl bg-(--foreground) overflow-hidden">
              ...
              >
                ...
                <ChevronDown
                  size={18}
                  className={`transition-transform duration-200 ${fusionExpanded ? "rotate-180" : ""}`}
                />
              </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-(--border)/20">
                          <div>
                            ...
                            <div className="text-lg font-semibold text-green-500">
                              {fusionData.dataItemMap?.active_power || 0} kW
                            </div>
                          </div>
                          <div>
                            <div className="text-xs text-(--text-muted)">
                            ...
                            <div className="text-lg font-semibold">
                              {fusionData.dataItemMap?.day_power || 0} kWh
                            </div>
                          </div>
                          <div>
                            <div className="text-xs text-(--text-muted)">
                            ...
                            <div className="text-lg font-semibold">
                              {fusionData.dataItemMap?.total_power || 0} kWh
                            </div>
                          </div>
                        </div>
                      )}
