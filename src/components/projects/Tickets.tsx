...
export default function Tickets({ client }: { client: string }) {
	...
	const load = async () => {
		try {
			setLoading(true);
			const [ticketsRes, usersRes] = await Promise.all([fetch(`/api/projects/tickets?client=${encodeURIComponent(client)}`), fetch('/api/users')]);
					...
					<Card key={ticket.id} className="overflow-hidden">
								...
								<div className="min-w-0">
									...
									<div className="text-xs text-(--text-muted) flex items-center gap-2 mt-1 flex-wrap min-w-0">
										...
										<span className="truncate min-w-0 flex-1">
											{new Date(ticket.createdAt).toLocaleString()} &bull; Opened by {ticket.openedBy}
										</span>
									</div>
								</div>
							</div>
						...
						{expandedTickets[ticket.id] && (
							<div className="divide-y divide-(--border)/10 min-w-0">
								...
								{ticket.pois.length === 0 && <div className="p-4 text-sm text-(--text-muted) text-center">No points of interest for this ticket.</div>}
							</div>
						)}
					</Card>
				))}
		...
		<div ref={setNodeRef} style={style} className="p-4 flex flex-col lg:flex-row gap-4 lg:items-center justify-between min-w-0 bg-(--background) relative">
			<div className="flex items-start gap-3 flex-1 min-w-0">
				...
				<div className="flex items-center gap-2 mt-1 shrink-0">
					{isAllowed && (
						<button onClick={() => setPoiToEdit({ poi, ticket })} className="text-(--text-muted) hover:text-(--accent) transition-colors p-1" title="Edit Task">
							<Edit3 size={16} />
						</button>
					)}
