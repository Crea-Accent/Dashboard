...
export default function Tickets({ client }: { client: string }) {
  ...
  const [poiToComplete, setPoiToComplete] = useState<{
    ticketId: string;
    poiId: string;
    requiresPicture?: boolean;
  } | null>(null);
  ...
  const handleDragEnd = async (event: any, ticketId: string) => {
    ...
    if (over && active.id !== over.id) {
      ...
      const newPOIs = arrayMove(ticket.pois, oldIndex, newIndex).map(
        (p, i) => ({ ...p, importance: i + 1 }),
      );
        ...
        setExpandedTickets((curr) => {
          ...
          loaded.forEach((t: Ticket) => {
            if (next[t.id] === undefined) {
              const isCompleted =
                t.pois.length > 0 &&
                t.pois.every((p) => p.state === "finished");
  ...
  const markPOIDone = async (
    ...
    const ticket = tickets[ticketIndex];
    const updatedPOIs = ticket.pois.map((poi) =>
      poi.id === poiId
        ? {
            ...poi,
    ...
    const isCompleted =
      updatedPOIs.length > 0 &&
      updatedPOIs.every((p) => p.state === "finished");
      ...
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          if (ctx) {
            ...
            resolve({
              data: canvas.toDataURL(isPng ? "image/png" : "image/jpeg", 0.9),
    ...
    try {
      ...
      const renderTable = (pois: any[], title: string) => {
        ...
        const tableData = pois.map((poi: any) => [
          ...
          poi.description,
          poi.state === "finished"
            ? poi.completedBy || poi.technician || "Unknown"
        ...
        autoTable(doc, {
          ...
          head: [
            [
              "#",
              "Description",
              title === "Completed Tasks" ? "Completed By" : "Assignee",
              "Notes",
            ],
          ],
          body: tableData,
      ...
      const unfinishedPOIs = ticket.pois.filter(
        (p: any) => p.state !== "finished",
      );
      const finishedPOIs = ticket.pois.filter(
        (p: any) => p.state === "finished",
      );
      ...
      const poisWithImages = ticket.pois.filter(
        (poi: any) => poi.imagePath || poi.finishedImagePath,
      );
      ...
      if (poisWithImages.length > 0) {
        ...
        for (let i = 0; i < poisWithImages.length; i++) {
          ...
          doc.text(
            `Task #${ticket.pois.findIndex((p: any) => p.id === poi.id) + 1}`,
            14,
            currY,
          );
          ...
          if (poi.imagePath) {
            beforeImg = await getBase64Image(
              `/api/files/render?path=${encodeURIComponent(poi.imagePath)}`,
            );
          }
          if (poi.finishedImagePath) {
            afterImg = await getBase64Image(
              `/api/files/render?path=${encodeURIComponent(poi.finishedImagePath)}`,
            );
          }
      ...
      doc.save(
        `${ticket.name || "Summary"}_${new Date().toISOString().split("T")[0]}.pdf`,
      );
    } catch (error) {
  ...
  const generateProjectPDF = async () => {
    ...
    try {
      ...
      for (let tIndex = 0; tIndex < tickets.length; tIndex++) {
        ...
        doc.setTextColor(lightText[0], lightText[1], lightText[2]);
        doc.text(
          `Opened by: ${ticket.openedBy || "Unknown"} - ${new Date(ticket.createdAt).toLocaleDateString()}`,
          14,
          currentY + 6,
        );
      ...
      doc.save(
        `${client}_Project_Summary_${new Date().toISOString().split("T")[0]}.pdf`,
      );
    } catch (error) {
    ...
    <section className="space-y-4">
      ...
      {tickets.length === 0 && (
        <EmptyState
          ...
          description="Create a new ticket to track points of interest."
        />
      )}
          ...
          <Card key={ticket.id} className="overflow-hidden">
            <div
              ...
              onClick={() =>
                setExpandedTickets((curr) => ({
                  ...curr,
                  [ticket.id]: !curr[ticket.id],
                }))
              }
            >
              <div className="min-w-0 flex items-center gap-3">
                <ChevronDown
                  className={`shrink-0 text-(--text-muted) transition-transform ${expandedTickets[ticket.id] ? "rotate-180" : ""}`}
                  size={20}
                />
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-(--text-muted) flex items-center gap-2 mt-1 flex-wrap min-w-0">
                    ...
                    <span className="truncate min-w-0 flex-1">
                      {new Date(ticket.createdAt).toLocaleString()} &bull;
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 shrink-0">
                <div className="text-sm font-medium shrink-0">
                  {ticket.pois.filter((p) => p.state === "finished").length} /{" "}
                  {ticket.pois.length} Finished
                </div>
                {isAllowed && (
                  <Button
                    size="sm"
              ...
              <div className="divide-y divide-(--border)/10 min-w-0">
                ...
                {ticket.pois.length === 0 && (
                  <div className="p-4 text-sm text-(--text-muted) text-center">
                    No points of interest for this ticket.
                  </div>
                )}
              </div>
            )}
      ...
      {poiToEdit && (
        <EditTaskModal
          ...
          task={{
            ...poiToEdit.poi,
            ticketId: poiToEdit.ticket.id,
            ticketName: poiToEdit.ticket.name || "",
            projectName: client,
          }}
...
function SortablePOI({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="p-4 flex flex-col lg:flex-row gap-4 lg:items-center justify-between min-w-0 bg-(--background) relative"
    >
      <div className="flex items-start gap-3 flex-1 min-w-0">
        {isAllowed && (
          <div
            {...attributes}
            {...listeners}
            className="mt-1 cursor-grab active:cursor-grabbing text-(--border)/40 hover:text-(--text-muted) transition-colors shrink-0 touch-none"
          >
        ...
        <div className="flex items-center gap-2 mt-1 shrink-0">
          {isAllowed && (
            <button
              onClick={() => setPoiToEdit({ poi, ticket })}
          ...
          <button
            disabled={
              poi.state === "finished" ||
              (poi.technician !== currentUsername && !isAllowed)
            }
            onClick={() =>
              setPoiToComplete({
                ticketId: ticket.id,
        ...
        <div className="flex-1 min-w-0">
          <p
            className={`text-sm break-words whitespace-normal ${poi.state === "finished" ? "line-through text-(--text-muted)" : ""}`}
          >
            {poi.description}
          </p>
          <div className="flex flex-wrap items-center gap-2 mt-2 text-xs min-w-0">
            ...
            <span className="px-2 py-0.5 rounded-full capitalize shrink-0 bg-zinc-500/10 text-zinc-500 font-mono text-[10px]">
              #
              {poi.importance ||
                ticket.pois.findIndex((p) => p.id === poi.id) + 1}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3 shrink-0 flex-col sm:flex-row w-full lg:w-auto min-w-0">
        {poi.imagePath && (
          <div className="w-full sm:w-48 min-w-0">
            <File
              file={{
                name: poi.imagePath.split("/").pop() || "POI.jpg",
                path: poi.imagePath,
                type: "file",
              }}
              image
        ...
        {poi.finishedImagePath && (
          <div className="relative w-full sm:w-48 overflow-hidden min-w-0 mt-3 sm:mt-0">
            <File
              file={{
                name: poi.finishedImagePath.split(/[/\\]/).pop() || "Proof.jpg",
                path: poi.finishedImagePath,
                type: "file",
              }}
              image
            />
