...
export default function File({
  ...
  const extension =
    file.type === "directory" ? "" : (file.name.split(".").pop() ?? "");
  ...
  const uploader =
    users.find((u) => getInitials(u.name as string) === uploaderInitials)
      ?.name ?? uploaderInitials;
  ...
  const collaborators = rawCollaborators
    ...
    .map((initials) => {
      return (
        users.find((u) => getInitials(u.name as string) === initials)?.name ??
        initials
      );
    });
  ...
  const isImage =
    image || ["png", "jpg", "jpeg", "gif", "webp", "svg"].includes(extension);
      ...
      <>
        ...
        >
          <button
            ...
            className="group aspect-video overflow-hidden w-full max-h-50 bg-black/5 flex items-center justify-center block cursor-zoom-in"
          >
            <div className="transition-transform duration-200 group-hover:scale-105 w-full h-full flex items-center justify-center">
              {isVideo ? (
                <video
  ...
  if (compact)
    return (
      <>
        <div
          className="rounded-3xl bg-(--foreground)  px-4 py-3 flex"
          draggable={isAllowed || false}
          onDragStart={() => onDragStart?.(file)}
          onDoubleClick={handleOpen}
          onContextMenu={handleContextMenu}
  ...
  return (
    <div
      className="rounded-3xl bg-(--foreground) p-5"
      draggable={isAllowed || false}
      onDragStart={() => onDragStart?.(file)}
      onDoubleClick={handleOpen}
      onContextMenu={handleContextMenu}
