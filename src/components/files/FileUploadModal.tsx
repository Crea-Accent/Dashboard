...
type Props = {
	...
	initialGroup?: string;
	onClose: () => void;
	onUpload: (file: File, group: string | undefined, name: string, comment: string, collaborators: string[]) => Promise<void>;
	users: User[];
};
...
export default function FileUploadModal({ files, open, groups = [], initialGroup, onClose, onUpload, users }: Props) {
	...
	const uploadAll = async () => {
		try {
			...
			for (const entry of entries) {
				await onUpload(entry.file, entry.group, entry.name, entry.comment, entry.collaborators);
			}
				...
				<div className="overflow-y-auto p-5 space-y-4">
					{entries.map((entry, index) => (
						<div key={`${entry.file.name}-${index}`} className="p-4 space-y-4">
							...
							<div className={`grid gap-4 ${groups.length > 0 ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
								<Input
									label="Display Name"
									value={entry.name}
									onChange={(e: any) =>
								...
								{groups.length > 0 && (
									<div>
										<label className="block text-sm font-medium mb-2">Destination Group</label>
										<Selector
											value={entry.group || 'Ungrouped'}
											options={[{ label: 'None (Ungrouped)', value: 'Ungrouped' }, ...groups.filter((g) => g !== 'Ungrouped').map((g) => ({ label: g, value: g }))]}
											onChange={(val) =>
												updateEntry(index, {
													group: val,
												})
